...
| --- | --- | --- | --- |
| **Plan hotspots** | Imported kitchens: `kitchen.hotspots` in DB (from PDF+Excel import). Legacy seed kitchens: `frontend/components/kitchen-svg-stage.jsx` → `IMAGE_HOTSPOTS_BY_SLUG` | `%` of plan image (`left`, `top`, `width`, `height`) | Green/blue overlay on the plan when selecting |
| **Catalog dimensions** | `frontend/prisma/seed.js` or import → `widthMm`, `heightMm`, `depthMm` on each item | millimetres | `W x H x D mm` line under the article number in the catalog |
...
## Automated import path (preferred for new kitchens)

When importing via **Admin → Import Kitchen (PDF + Excel)**:

1. Excel **`NR`** column must match invisible white callout numbers on the PDF text layer.
2. `docs/detect-plan-callouts.py` extracts each NR position.
3. `frontend/lib/kitchen-import-callout-mapping.js` maps NR → `componentKey` by wall/base band and left-to-right order.
4. `docs/build-hotspots-json.py` places one ink-snapped box per mapped callout (`build_boxes_per_callout_ink`).
5. Hotspots are stored on the kitchen record (`kitchen.hotspots`); the configurator uses those automatically.

**No layout template kitchen is required** when NR callouts are present. Use the manual measurement workflow below only when auto hotspots fail overlay/calibrate checks.

---

## Plan hotspot measurement workflow (manual / refinement)
...
Extract callout positions (links Excel NR to plan):

```bash
python docs/detect-plan-callouts.py "frontend/public/pdfs/AB <plan>.pdf"
```
...
Seed/import: hood cabinet → `iconKey: "hood_wall_cabinet"`; hidden hood row → `isActive: false`.
...
- Always **locked** (`isLocked: true` in seed/import).
...
## Catalog dimension rules (Excel → seed / import)
...
Always set these on the item. The catalog renders them via `getStructuredDimensions()` in `kitchen-catalog-panel.jsx` as a line under the article:
...
| `frontend/prisma/seed.js` | `<PLAN>_ITEMS`, `DEFAULT_KITCHENS`, `DEFAULT_KITCHEN_CONTRACTS` (seed path only) |
| `frontend/components/kitchen-svg-stage.jsx` | `IMAGE_VIEW_BY_SLUG` + `IMAGE_HOTSPOTS_BY_SLUG` (legacy seed path only) |
| `frontend/components/kitchen-selection-utils.js` | Callout numbers for new codes, `LINKED_COMPONENT_GROUPS_BY_SLUG` |
| Admin import | PDF + Excel → `kitchen.hotspots` in DB (preferred for new kitchens) |
...
- [ ] `npx prisma db seed` succeeds (seed path).
...
2. Re-run `detect-plan-hotspots.py` and `detect-plan-callouts.py`.
3. Compare new grid to existing hotspots — if geometry unchanged, assets-only update is enough.
4. If layout shifted, re-measure and update hotspots using the rules above (or re-import PDF+Excel).
5. Catalog data usually unchanged unless Excel/items changed.
...
Requirements:
- Vector SVG plan + pixel-perfect hotspots (% — DB import or kitchen-svg-stage.jsx for seed kitchens)
...
- Run prisma db seed (seed path) or re-import PDF+Excel (import path) and test in browser
```
