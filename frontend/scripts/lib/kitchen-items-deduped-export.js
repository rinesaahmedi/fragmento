...
function writeWorkbook(rows, outPath, sheetName) {
...
const AB_KITCHEN_EXPORT_EXTRA_SLUGS = ["108134-modul-1"];

function isAbKitchenExportSlug(slug) {
  const normalized = String(slug || "").trim().toLowerCase();
  return normalized.startsWith("ab-") || AB_KITCHEN_EXPORT_EXTRA_SLUGS.includes(normalized);
}

function abKitchenExportWhere(includeInactive, KitchenStatus) {
  return {
    OR: [
      { slug: { startsWith: "ab-" } },
      ...AB_KITCHEN_EXPORT_EXTRA_SLUGS.map((slug) => ({ slug })),
    ],
    ...(includeInactive ? {} : { status: KitchenStatus.ACTIVE }),
  };
}

function otherKitchenExportWhere(includeInactive, KitchenStatus) {
  return {
    AND: [
      { NOT: { slug: { startsWith: "ab-" } } },
      ...AB_KITCHEN_EXPORT_EXTRA_SLUGS.map((slug) => ({ NOT: { slug } })),
      ...(includeInactive ? [] : [{ status: KitchenStatus.ACTIVE }]),
    ],
  };
}
