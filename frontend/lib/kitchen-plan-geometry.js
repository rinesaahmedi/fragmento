...
const BASE_PLINTH_EXTENSION_DISABLED_SLUGS = new Set([
  ...
  "ab-105859",
  "ab-105862",
  // Perspective drawings already include each cabinet's drawn bottom.
  "ab-105837",
  "ab-105840",
  "ab-105843",
  "ab-105831",
...
export function isHorizontalWorktopHotspot(definition) {
...
export function isPlanHighlightHotspot(hotspot) {
  ...
  if (hotspot.componentKey === "worktop") {
    ...
    if (bounds.height >= CORNER_BLENDE_MIN_HEIGHT && bounds.width <= CORNER_BLENDE_MAX_WIDTH + 0.2) return false;
    if (bounds.height > 10 && bounds.width < 2.5) return false;
  }
  return true;
}
...
export function withBasePlinthExtension(definitions, slug) {
  ...
  if (BASE_PLINTH_EXTENSION_DISABLED_SLUGS.has(normalizedSlug)) return definitions;
  ...
  if (gapToFloor <= BASE_PLINTH_ALREADY_INCLUDED_GAP) return definitions;

  const targetBottom = Math.min(bodyBottom + BASE_PLINTH_EXTENSION_PERCENT, PLAN_DIMENSION_LINE_PERCENT - 1);
  return definitions.map((definition) => {
    if (!isBaseBodyHotspot(definition)) return definition;
    const currentBottom = definition.top + definition.height;
    if (currentBottom >= targetBottom - 0.2) return definition;
    return {
      ...definition,
      height: targetBottom - definition.top,
      claimOriginalBodyHeightRatio: definition.height / Math.max(targetBottom - definition.top, 0.0001),
    };
  });
}
...
export function getPlanDisplayCrop(hotspots, slug) {
  ...
  const bounds = hotspots.reduce((current, hotspot) => {
    const next = getHotspotSourceBounds(hotspot);
    return {
  ...
  const cropTuning = PLAN_DISPLAY_CROP_TUNING_BY_SLUG[normalizeSlug(slug)] || {};
  ...
  const right = clampPercent(Math.min(rightLimit, bounds.right + Math.max(3.2, trailingX * 0.92)));
...
export function cropPlanHotspot(hotspot, crop) {
  ...
  if (points.length) {
    ...
    const clipPathPoints = croppedPoints.map(([x, y]) => {
...
export function prepareKitchenPlanGeometry(hotspots, slug, components = [], options = {}) {
  const sourceDefinitions = (hotspots || []).map(withHotspotSourceBounds);
  const prepared = withCornerBlendeExtensions(withDerivedSinkFaucet(sourceDefinitions, components));
  return options.includeBasePlinth === false ? prepared : withBasePlinthExtension(prepared, slug);
}
